...
$(function () {
    $.ajax({
        type: "post",
        url: httpXhr + "informationController/appselectInformation.do?f_id=" + aid,
        dataType: "jsonp",
        jsonp: "jsonpCallback",
        success: function (data) {
            if (data.code == "200") {
                var json = data.data;
                // 标题
                $(".content_tit h5").html(json.f_name);
                // 内容
                $(".nav_content").html(json.f_des);
                $(".nav_content img").css("width", "100%");
                $(".nav_content h4").css("font-size", "15px");
                // 来源
                $(".list_nav>p:nth-of-type(1)").html(json.f_source);
                // 时间
                $(".list_nav>p:nth-of-type(2)").html(json.c_date.substring(0, 10));
                // 浏览量
