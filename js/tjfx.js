...
var aid = GetQueryString("c_id"); //课程ID
...
$(document).ready(function () {
    ...
    var listColorArr = ['#B1D9D9', '#F5CCCA', '#7FC591', '#FBCB03', '#74BB5F', '#BDA4CD', '#D05A80', '#EEAE94', '#A0BADB', '#43ADDD', '#F6CCCA', '#EDAA3E', '#B56845', '#DB4343', '#982E2B', '#9E774E', '#4B4D8C', '#B7CB75', '#2D83BC', '#E78548', '#43AEDD', '#7B6E67'];
    for (var i = 0; i < listColorArr.length; i++) {
        $(".list" + (i + 1) + "").css("background", listColorArr[i]);
    }

    // 社区·move.数据
    $(".move_list").hover(function () {
        $(".month_count>span").html("全部");
        var move_list = $(this).attr("value");
        $(".sq_gd>img").attr("src", "images/map/" + move_list + ".png");
        $(".sq_gd,.sq_bg").show();
        //init Chart 圆环图
        higtChart_cril(move_list);
        //init Chart 柱状图
        higtChart_z(move_list);
        //荣誉之星
        ryzx(move_list);
        //督导员
        ddy(move_list);
...
function higtChart_cril(move_list) {
    var htp = "";
    if (move_list == "" || move_list == undefined || move_list == null) {
        htp = httpXhr + "areasController/selectZXL.do";
    } else {
        htp = httpXhr + "areasController/selectZXL.do?pc=1&a_id=" + move_list;
    }
    // 知晓率
    $.ajax({
        type: "post",
...
function higtChart_z(move_list) {
    var htp = "";
    if (move_list == "" || move_list == undefined || move_list == null) {
        htp = httpXhr + "activityController/activityCount.do";
    } else {
        htp = httpXhr + "activityController/activityCount.do?pc=1&a_id=" + move_list;
    }
    $.ajax({
        type: "post",
        url: htp,
                    ...
                    plotOptions: {
                        column: {
                            ...
                            events: {
                                ...
                                click: function (e) {
                                    location.href = "tjfx_list.jsp?a_id=" + e.point.id+"&index=6&list=6";
                                },
                            },
                        }
...
function higtChart_month(month_id) {
    ...
    $.ajax({
        type: "post",
        url: htp = httpXhr + "activityController/activityCount.do?pc=1&month=" + month_id,
        dataType: "json",
        success: function (data) {
            if (data.code == 200) {
                var json = data.data;
...
function ryzx(move_list) {
    ...
    var htp = "";
    if (move_list == "" || move_list == undefined || move_list == null) {
        htp = httpXhr + "userController/usersList.do";
    } else {
        htp = httpXhr + "userController/usersList.do?a_id=" + move_list;
    }
    $.ajax({
        type: "post",
        url: htp,
...
$(".move_list").click(function () {
    var move_list = $(this).attr("value");
    var move_name = $("." + move_list + ">span").html();
    location.href = "tjfx_detail.jsp?sq_id=" + move_list + "&move_name=" + encodeURI(encodeURI(move_name)) + "&index=6";
});
