...
$(function () {
    ...
    $.ajax({
        type: "post",
        url: httpXhr + "littleKnowledgeController/appselectLittleKnowledge.do?l_id=" + aid,
        dataType: "jsonp",
        jsonp: "jsonpCallback",
        success: function (data) {
            ...
            //     var img = encodeURI(encodeURI(json[i].f_address));
            ...
            if (data.code == "200") {
                // 标题
                $(".content_tit h5").html(data.data.l_title);
                // 内容
                $(".nav_content").html(data.data.l_centent);
                $(".nav_content img").css("width", "100%");
                $(".nav_content h4").css("font-size", "15px");
                // 来源
                $(".list_nav>p:nth-of-type(1)").html(data.data.l_source);
                // 时间
                $(".list_nav>p:nth-of-type(2)").html(data.data.l_date.substring(0, 10));
                // 浏览量
                $(".list_ck>span").html(data.data.lookNum);
                // 点赞
                $(".list_dz>span").html(data.data.dzNum);
